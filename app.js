...
function pinModal(action, setup=false){
  return new Promise((resolve,reject)=>{
    ...
    modal.innerHTML=`<div class="pin-modal" role="dialog" aria-modal="true" aria-labelledby="pin-modal-title"><div class="pin-modal-icon">🔐</div><h2 id="pin-modal-title">${escapeHtml(title)}</h2><p>${escapeHtml(subtitle)}</p><div class="pin-modal-field"><label for="pin-modal-input">${setup?'New PIN':'Personal PIN'}</label><div class="pin-modal-input-wrap"><input id="pin-modal-input" class="inventro-pin-field" type="text" inputmode="numeric" maxlength="8" autocomplete="off" name="inventro_pin" placeholder="4–8 digits"><button type="button" id="pin-modal-toggle" aria-label="Show PIN">👁</button></div></div>${setup?'<div class="pin-modal-field"><label for="pin-modal-confirm">Confirm PIN</label><div class="pin-modal-input-wrap"><input id="pin-modal-confirm" class="inventro-pin-field" type="text" inputmode="numeric" maxlength="8" autocomplete="off" name="inventro_pin_confirm" placeholder="Re-enter PIN"><button type="button" data-confirm-toggle aria-label="Show PIN">👁</button></div></div>':''}<div class="pin-modal-actions"><button type="button" class="small-action" id="pin-modal-cancel">Cancel</button><button type="button" class="btn btn-primary" id="pin-modal-submit">${setup?'Create PIN':'Verify PIN'}</button></div><div class="pin-modal-error" id="pin-modal-error" hidden></div></div>`;
...
function showPinGate(){
  ...
  root.innerHTML=`<div class="screen pin-gate"><div class="pin-gate-card"><p class="brand">Inventro Security</p><div class="pin-icon-wrap">${canBiometric?'👆':'🔒'}</div><h1>${has?'Workspace locked':'Create your security PIN'}</h1><p class="subtitle">${has?(canBiometric?'Use your fingerprint to continue, or enter your PIN below.':'Enter your personal PIN to continue. The same PIN works on any device you sign into.'):'Choose your own 4–8 digit PIN. You will need it when the app opens and after 5 minutes of inactivity.'}</p>${canBiometric?'<button type="button" class="btn btn-primary" id="biometric-retry">👆 Use fingerprint</button><p class="pin-hint">Or enter your PIN instead:</p>':''}<div class="pin-form"><div class="field"><label for="inventro-pin">${has?'PIN':'New PIN'}</label><div class="pin-input-wrap"><input id="inventro-pin" class="inventro-pin-field" type="text" inputmode="numeric" maxlength="8" autocomplete="off" name="inventro_pin" placeholder="Enter PIN"><button type="button" class="pin-toggle" data-target="inventro-pin" aria-label="Show PIN">👁</button></div></div>${has?'':'<div class="field"><label for="inventro-pin-confirm">Confirm PIN</label><div class="pin-input-wrap"><input id="inventro-pin-confirm" class="inventro-pin-field" type="text" inputmode="numeric" maxlength="8" autocomplete="off" name="inventro_pin_confirm" placeholder="Re-enter PIN"><button type="button" class="pin-toggle" data-target="inventro-pin-confirm" aria-label="Show PIN">👁</button></div></div>'}<button class="btn btn-primary" id="unlock-pin">${has?'Unlock Inventro':'Create PIN & Enter'}</button></div><p class="pin-hint">Your PIN is personal and now syncs to your account, not just this device.</p></div></div>`;
...
async function cancelStockRequest(requestId){
  const companyId=currentCompanyId(), user=auth.currentUser;
  if(!companyId||!user) throw new Error('Your company session is not available.');
  if(!canCreateRequest()) throw new Error('Only a request creator can cancel a request.');
...
// Additional UI strings used by the feature screens.
// Keep this table separate so newly added screen labels are translated too.
Object.assign(BN, {
  'Kitchen inventory, simplified.':'কিচেন ইনভেন্টরি, আরও সহজ।',
  'Secure workspace':'নিরাপদ ওয়ার্কস্পেস',
  'Smart stock management':'স্মার্ট স্টক ম্যানেজমেন্ট',
  'Everything your kitchen needs,':'আপনার কিচেনের প্রয়োজনীয় সবকিছু,',
  'in one place.':'এক জায়গায়।',
  'Create a company':'কোম্পানি তৈরি করুন',
  'Join a company':'কোম্পানিতে যোগ দিন',
  '✓ Google sign-in':'✓ Google সাইন-ইন',
  '✓ Live stock sync':'✓ লাইভ স্টক সিঙ্ক',
  '✓ Role-based access':'✓ রোল-ভিত্তিক অ্যাক্সেস',
  'Inventory overview':'ইনভেন্টরি ওভারভিউ',
  'Today’s kitchen stock':'আজকের কিচেন স্টক',
  'Everything under control.':'সবকিছু নিয়ন্ত্রণে।',
  '● LIVE':'● লাইভ',
  'Total items':'মোট আইটেম',
  'Across your kitchen':'পুরো কিচেন জুড়ে',
  'Needs attention':'মনোযোগ প্রয়োজন',
  'available':'উপলভ্য',
  'Healthy':'ভালো',
  'Daily receiving':'দৈনিক স্টক গ্রহণ',
  'Stock synced':'স্টক সিঙ্ক হয়েছে',
  'Just now':'এইমাত্র',
  'requests':'রিকোয়েস্ট',
  'One inventory':'একটি ইনভেন্টরি',
  'Built for teams':'টিমের জন্য তৈরি',
  'Clear insights':'স্পষ্ট তথ্য',
  'Designed for busy kitchens · Simple enough for everyone':'ব্যস্ত কিচেনের জন্য তৈরি · সবার জন্য সহজ',
  'You’re set up':'সেটআপ সম্পন্ন হয়েছে',
  'You\'re set up':'সেটআপ সম্পন্ন হয়েছে',
  'Save this code':'এই কোডটি সেভ করুন',
  'Your company\'s join code':'আপনার কোম্পানির যোগদানের কোড',
  'Enter workspace':'ওয়ার্কস্পেসে প্রবেশ করুন',
  '6-digit code':'৬ সংখ্যার কোড',
  '↻ Change Google account':'↻ Google অ্যাকাউন্ট পরিবর্তন করুন',
  'Inventro Employee Access':'Inventro কর্মী অ্যাক্সেস',
  'Enter company code':'কোম্পানি কোড লিখুন',
  '6-digit company code':'৬ সংখ্যার কোম্পানি কোড',
  'Select department…':'ডিপার্টমেন্ট নির্বাচন করুন…',
  'Select department...':'ডিপার্টমেন্ট নির্বাচন করুন...',
  'Inventory setup':'ইনভেন্টরি সেটআপ',
  'Automatic item photo':'অটোমেটিক আইটেম ছবি',
  'Inventro will try to find a product image when you save.':'সেভ করার সময় Inventro পণ্যের ছবি খুঁজে নেওয়ার চেষ্টা করবে।',
  'NEW':'নতুন',
  'Supplier reorder':'সাপ্লায়ার রিঅর্ডার',
  'Open':'খোলা',
  'Build supplier order':'সাপ্লায়ার অর্ডার তৈরি করুন',
  'Red items are included automatically. Select yellow items. Ordered items can be selected again only for resend.':'লাল আইটেম স্বয়ংক্রিয়ভাবে যোগ হবে। হলুদ আইটেম নির্বাচন করুন। অর্ডার করা আইটেম শুধু পুনরায় পাঠানোর জন্য আবার নির্বাচন করা যাবে।',
  '🔴 New red':'🔴 নতুন লাল',
  '🟡 Yellow':'🟡 হলুদ',
  '🔵 Ordered':'🔵 অর্ডার করা',
  ...
  'Note (optional)':'নোট (ঐচ্ছিক)',
  'Close outstanding supplier order':'বকেয়া সাপ্লায়ার অর্ডার বন্ধ করুন',
  'If this delivery is the order that was previously sent, mark it as received/fulfilled.':'এই ডেলিভারিটি আগে পাঠানো অর্ডারের হলে সেটিকে গৃহীত/সম্পন্ন হিসেবে চিহ্নিত করুন।',
  'No matching stock item found.':'মিল থাকা কোনো স্টক আইটেম পাওয়া যায়নি।',
  'Select a stock item from the dropdown above.':'উপরের ড্রপডাউন থেকে একটি স্টক আইটেম নির্বাচন করুন।',
  'Stock now':'বর্তমান স্টক',
  'Low limit':'লো লিমিট',
  'Last dispatch':'শেষ ডিসপ্যাচ',
  'Last arrival':'শেষ আগমন',
  'Supplier order outstanding':'সাপ্লায়ার অর্ডার বকেয়া',
  'Mark read':'পড়া হিসেবে চিহ্নিত করুন',
  'No notifications':'কোনো নোটিফিকেশন নেই',
  'Your request updates will appear here.':'আপনার রিকোয়েস্টের আপডেট এখানে দেখা যাবে।',
  'Kitchen workflow':'কিচেন ওয়ার্কফ্লো',
  'Stock Requests':'স্টক রিকোয়েস্ট',
  'New request':'নতুন রিকোয়েস্ট',
  'Quantity needed':'প্রয়োজনীয় পরিমাণ',
  'Reason / note':'কারণ / নোট',
  'Send request':'রিকোয়েস্ট পাঠান',
  '📝 My requests':'📝 আমার রিকোয়েস্ট',
  'My requests':'আমার রিকোয়েস্ট',
  'My notifications':'আমার নোটিফিকেশন',
  'Request status updates and stock dispatch notifications.':'রিকোয়েস্ট স্ট্যাটাস আপডেট ও স্টক ডিসপ্যাচ নোটিফিকেশন।',
  'Track the requests you have submitted and their current status.':'আপনার পাঠানো রিকোয়েস্ট ও বর্তমান স্ট্যাটাস দেখুন।',
  'Filter by date':'তারিখ অনুযায়ী ফিল্টার',
  'Show all dates':'সব তারিখ দেখুন',
  'Approved requests stay yellow until dispatched. Dispatched requests turn green.':'অনুমোদিত রিকোয়েস্ট ডিসপ্যাচ না হওয়া পর্যন্ত হলুদ থাকবে। ডিসপ্যাচ হলে সবুজ হবে।',
  'Department:':'ডিপার্টমেন্ট:',
  'Cancel request':'রিকোয়েস্ট বাতিল করুন',
  'No requests for this date':'এই তারিখে কোনো রিকোয়েস্ট নেই',
  'Try another date or show all dates.':'অন্য তারিখ চেষ্টা করুন অথবা সব তারিখ দেখুন।',
  'Procurement':'প্রোকিউরমেন্ট',
  '🛒 Supplier order list':'🛒 সাপ্লায়ার অর্ডার লিস্ট',
  'Add another inventory item…':'আরও একটি ইনভেন্টরি আইটেম যোগ করুন…',
  '＋ Add item':'＋ আইটেম যোগ করুন',
  'No order items yet. Add an item above.':'এখনও কোনো অর্ডার আইটেম নেই। উপরে একটি আইটেম যোগ করুন।',
  '💾 Save list':'💾 লিস্ট সেভ করুন',
  '📄 Generate PDF & Share':'📄 PDF তৈরি ও শেয়ার করুন',
  'Final saved list':'চূড়ান্ত সেভ করা লিস্ট',
  'Supplier orders':'সাপ্লায়ার অর্ডার',
  'Supplier Order List':'সাপ্লায়ার অর্ডার লিস্ট',
  'Select a date.':'একটি তারিখ নির্বাচন করুন।',
  'Loading order list…':'অর্ডার লিস্ট লোড হচ্ছে…',
  'Last transaction':'শেষ ট্রানজ্যাকশন',
  'Transactions':'ট্রানজ্যাকশন',
  'Receiving & dispatch entries':'রিসিভ ও ডিসপ্যাচ এন্ট্রি',
  'Items moved':'সরানো আইটেম',
  'Different goods':'বিভিন্ন পণ্য',
  'Good':'ভালো',
  'Opening':'ওপেনিং',
  'Closing':'ক্লোজিং',
  'No receiving or dispatch activity for this day.':'এই দিনে কোনো রিসিভ বা ডিসপ্যাচ কার্যকলাপ নেই।',
  'New activity will appear here automatically.':'নতুন কার্যকলাপ এখানে স্বয়ংক্রিয়ভাবে দেখা যাবে।',
  'Transaction feed':'ট্রানজ্যাকশন ফিড',
  'View changes':'পরিবর্তন দেখুন',
  'No transactions recorded yet.':'এখনও কোনো ট্রানজ্যাকশন রেকর্ড করা হয়নি।',
  'Loading transaction changes':'ট্রানজ্যাকশনের পরিবর্তন লোড হচ্ছে',
  'Please wait while the audit record is loaded.':'অডিট রেকর্ড লোড হওয়া পর্যন্ত অপেক্ষা করুন।',
  'Transaction not found':'ট্রানজ্যাকশন পাওয়া যায়নি',
  'This transaction may have been deleted or is no longer available.':'এই ট্রানজ্যাকশনটি ডিলিট করা হয়েছে অথবা আর উপলভ্য নেই।',
  'You do not have permission to view the changes for this transaction.':'এই ট্রানজ্যাকশনের পরিবর্তন দেখার অনুমতি আপনার নেই।',
  'CURRENT SAVED DATA':'বর্তমানে সেভ করা ডেটা',
  'Type':'ধরন',
  'Changed by':'পরিবর্তন করেছেন',
  'Previous quantity':'আগের পরিমাণ',
  'Previous type':'আগের ধরন',
  'Previous department':'আগের ডিপার্টমেন্ট',
  'Previous note':'আগের নোট',
  'Original owner':'মূল মালিক',
  'No edit or delete revision has been recorded for this transaction.':'এই ট্রানজ্যাকশনের কোনো এডিট বা ডিলিট সংশোধন রেকর্ড করা হয়নি।',
  '📘 Daily transaction statement':'📘 দৈনিক ট্রানজ্যাকশন স্টেটমেন্ট',
  'No transaction statement for this day.':'এই দিনের কোনো ট্রানজ্যাকশন স্টেটমেন্ট নেই।',
  'DELETED':'ডিলিট করা হয়েছে',
  'EDITED':'এডিট করা হয়েছে',
  'No matching log entries':'মিল থাকা কোনো লগ এন্ট্রি নেই',
  'Try another date or filter.':'অন্য তারিখ বা ফিল্টার চেষ্টা করুন।',
  'Daily log book':'দৈনিক লগবুক',
  'Receiving, dispatch & request fulfilment':'রিসিভ, ডিসপ্যাচ ও রিকোয়েস্ট সম্পন্নকরণ',
  'Requests, approvals & dispatched fulfilments':'রিকোয়েস্ট, অনুমোদন ও ডিসপ্যাচ সম্পন্নকরণ',
  'Transaction control & daily statements':'ট্রানজ্যাকশন কন্ট্রোল ও দৈনিক স্টেটমেন্ট',
  'Supplier Order Lists':'সাপ্লায়ার অর্ডার লিস্ট',
  'View what the Transaction Manager prepared for each day.':'ট্রানজ্যাকশন ম্যানেজার প্রতিদিন কী প্রস্তুত করেছেন তা দেখুন।',
  'View Inventory Manager receiving and dispatch activity.':'ইনভেন্টরি ম্যানেজারের রিসিভ ও ডিসপ্যাচ কার্যকলাপ দেখুন।',
  'Activity':'কার্যকলাপ',
  ...
  'Dispatched quantity':'ডিসপ্যাচ করা পরিমাণ',
  'Low-stock items':'লো-স্টক আইটেম',
  'Pending requests':'অপেক্ষমাণ রিকোয়েস্ট',
  'Approved requests':'অনুমোদিত রিকোয়েস্ট',
  'Dispatched requests':'ডিসপ্যাচ করা রিকোয়েস্ট',
  'Rejected requests':'বাতিল রিকোয়েস্ট',
  'Key performance charts':'মূল পারফরম্যান্স চার্ট',
  '📊 Top 8 items · dispatched vs received':'📊 শীর্ষ ৮ আইটেম · ডিসপ্যাচ বনাম রিসিভ',
  '🥧 Current stock health':'🥧 বর্তমান স্টকের অবস্থা',
  '📈 Last 2 days':'📈 শেষ ২ দিন',
  'Edit inventory item':'ইনভেন্টরি আইটেম এডিট করুন',
  'Finding a new photo…':'নতুন ছবি খোঁজা হচ্ছে…',
  '‹ Back to workspace':'‹ ওয়ার্কস্পেসে ফিরে যান',
  'Administration':'অ্যাডমিনিস্ট্রেশন',
  'Team management':'টিম ম্যানেজমেন্ট',
  'Employee Gmail':'কর্মীর Gmail',
  'Login access days':'লগইন অ্যাক্সেসের দিন',
  'Team members':'টিমের সদস্য',
  'No employees added yet':'এখনও কোনো কর্মী যোগ করা হয়নি',
  'Add your first employee above.':'উপরে আপনার প্রথম কর্মী যোগ করুন।',
  '➕ Add inventory item':'➕ ইনভেন্টরি আইটেম যোগ করুন',
  'Inventro will try to find an image when you save the item.':'আইটেম সেভ করার সময় Inventro ছবি খুঁজে নেওয়ার চেষ্টা করবে।',
  'Save inventory item':'ইনভেন্টরি আইটেম সেভ করুন',
  'Inventory permissions':'ইনভেন্টরি অনুমতি',
  'Item creation':'আইটেম তৈরি',
  'Admin only':'শুধু অ্যাডমিন',
  'Stock operations':'স্টক অপারেশন',
  'Admin + Inventory Manager':'অ্যাডমিন + ইনভেন্টরি ম্যানেজার',
  '✏️ Edit saved inventory':'✏️ সেভ করা ইনভেন্টরি এডিট করুন',
  'Department setup':'ডিপার্টমেন্ট সেটআপ',
  '🏢 Add department':'🏢 ডিপার্টমেন্ট যোগ করুন',
  'Department name':'ডিপার্টমেন্টের নাম',
  'Add department':'ডিপার্টমেন্ট যোগ করুন',
  'Available for dispatch, requests and history filters':'ডিসপ্যাচ, রিকোয়েস্ট ও হিস্ট্রি ফিল্টারে ব্যবহার করা যাবে',
  'No departments yet':'এখনও কোনো ডিপার্টমেন্ট নেই',
  'Add at least one department before staff can dispatch/request stock for it.':'স্টাফরা কোনো ডিপার্টমেন্টের জন্য স্টক ডিসপ্যাচ/রিকোয়েস্ট করার আগে অন্তত একটি ডিপার্টমেন্ট যোগ করুন।',
  'Company controls':'কোম্পানি কন্ট্রোল',
  'Company information':'কোম্পানির তথ্য',
  'Your role':'আপনার রোল',
  'Admin email':'অ্যাডমিন ইমেইল',
  'Company ID':'কোম্পানি ID',
  '🔄 Change company code':'🔄 কোম্পানি কোড পরিবর্তন করুন',
  'Danger zone':'ঝুঁকিপূর্ণ অংশ',
  'Delete company':'কোম্পানি ডিলিট করুন',
  'Item added successfully. You can view it in Stock.':'আইটেম সফলভাবে যোগ হয়েছে। এটি স্টকে দেখতে পারবেন।',
  'Record received stock':'রিসিভ করা স্টক রেকর্ড করুন',
  'Record dispatch':'ডিসপ্যাচ রেকর্ড করুন',
  'Checking PIN…':'PIN যাচাই করা হচ্ছে…',
  'Confirm PIN':'PIN নিশ্চিত করুন',
  'Or enter your PIN instead:':'অথবা আপনার PIN লিখুন:',
  'Your PIN is personal and now syncs to your account, not just this device.':'আপনার PIN ব্যক্তিগত এবং এখন শুধু এই ডিভাইস নয়, আপনার অ্যাকাউন্টের সঙ্গেও সিঙ্ক হয়।',
  '👆 Use fingerprint':'👆 ফিঙ্গারপ্রিন্ট ব্যবহার করুন',
  '🔐 Security PIN':'🔐 সিকিউরিটি PIN',
  '↪ Sign out':'↪ সাইন আউট',
  '● Active':'● সক্রিয়',
  '‹ Back':'‹ পিছনে',
...
async function getMovementRowsForDayOnCall(day){
  ...
  const rows=[];
  await Promise.all(items.map(async item=>{
    // Do not use a two-range Firestore query here. That query requires a
    // composite index on createdAt and was the source of the "The query requires
    // an index" error shown on Statistics. Each item already has a relatively
    // small movement subcollection, so read it and filter the two calendar
    // boundaries locally. This makes Statistics work immediately on every
    // company without requiring an index deployment.
    ...
    snap.docs.forEach(d=>{
      ...
      if(!created || Number.isNaN(created.getTime()) || created < start || created >= end) return;
      const byEmail=String(data.byEmail||'').toLowerCase();
      rows.push({
        id:d.id,
        itemId:item.id,
...
async function getStatsRequestsOnCall(){
  ...
  let createdSnap, updatedSnap;
  if (isCompanyWide) {
    // Company-wide statistics can use a single range filter. Keep the query
    // index-free: combining createdAt/updatedAt with another where clause is
    // unnecessary here and can trigger a composite-index requirement.
    [createdSnap, updatedSnap] = await Promise.all([
      getDocs(query(base, where('createdAt','>=',startDate))),
      getDocs(query(base, where('updatedAt','>=',startDate)))
    ]);
  } else {
    // Requester / Transaction Manager statistics must not combine
    // requestedByUid == ownUid with a createdAt/updatedAt range in the same
    // Firestore query. That combination requires a composite index and was
    // the exact source of the red "The query requires an index" banner.
    // A single-field requester query is sufficient; date filtering is done
    // locally after the authorized documents are returned.
    const ownSnap = await getDocs(query(base, where('requestedByUid','==',ownUid)));
    const ownDocs = ownSnap.docs.filter(d => {
      const data = d.data() || {};
      const created = data.createdAt?.toDate?.() || (data.createdAt ? new Date(data.createdAt) : null);
      const updated = data.updatedAt?.toDate?.() || (data.updatedAt ? new Date(data.updatedAt) : null);
      const createdMs = created && !Number.isNaN(created.getTime()) ? created.getTime() : 0;
      const updatedMs = updated && !Number.isNaN(updated.getTime()) ? updated.getTime() : 0;
      return Math.max(createdMs, updatedMs) >= startDate.getTime();
    });
    createdSnap = { docs: ownDocs };
    updatedSnap = { docs: [] };
  }
  const byId=new Map();
